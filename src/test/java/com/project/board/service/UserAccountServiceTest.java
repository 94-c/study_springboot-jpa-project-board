...
class UserAccountServiceTest {
    ...
    void givenExistentUserId_whenSearching_thenReturnsOptionalUserData() {
        // Given
        String username = "boom";
        given(userAccountRepository.findByEmail(username)).willReturn(Optional.of(createUserAccount(username)));
        ...
        then(userAccountRepository).should().findByEmail(username);
    }
    ...
    void givenNonexistentUserId_whenSearching_thenReturnsOptionalUserData() {
        ...
        given(userAccountRepository.findByEmail(username)).willReturn(Optional.empty());
    ...
    void givenUserParams_whenSaving_thenSavesUserAccount() {
        // Given
        UserAccount userAccount = createUserAccount("boom");
        UserAccount savedUserAccount = createSigningUpUserAccount("boom");
        given(userAccountRepository.save(any(UserAccount.class))).willReturn(savedUserAccount); // 인자를 any()로 수정
        ...
        then(userAccountRepository).should().save(any(UserAccount.class)); // 인자를 any()로 수정
    }
