...
import com.project.board.dto.response.admin.AdminArticleCommentClientResponse;
...
public class ArticleCommentManagementService {
    ...
    public List<AdminArticleCommentDto> getArticleComments() {
        URI uri = UriComponentsBuilder.fromHttpUrl(projectProperties.board().url() + "/api/articleComments")
                ...
                .build()
                .toUri();
        AdminArticleCommentClientResponse response = restTemplate.getForObject(uri, AdminArticleCommentClientResponse.class);

        return Optional.ofNullable(response).orElseGet(AdminArticleCommentClientResponse::empty).articleComments();
    }

    public AdminArticleCommentDto getArticleComment(Long articleCommentId) {
        URI uri = UriComponentsBuilder.fromHttpUrl(projectProperties.board().url() + "/api/articleComments/" + articleCommentId)
                ...
                .build()
                .toUri();
        AdminArticleCommentDto response = restTemplate.getForObject(uri, AdminArticleCommentDto.class);
